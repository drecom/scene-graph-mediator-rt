    ...
    Pixi.prototype.createContainer = function (node, resources) {
        ...
        if (node.spine) {
            ...
            if (node.sprite.atlasUrl) {
                ...
                if (node.sprite.url) {
                    object = new PIXI.mesh.NineSlicePlane(resources[node.sprite.url].texture, node.sprite.slice.left, node.sprite.slice.top, node.sprite.slice.right, node.sprite.slice.bottom);
